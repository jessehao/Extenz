...
public extension UITextView {
    func shouldTextIncrease(afterChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        return (range.location >= self.text.count && range.length > 0)
            || (range.location < self.text.count && range.length < text.count)
    }
    
    func shouldTextDecrease(afterChangeTextIn range: NSRange, replacementText text: String) -> Bool {
        return range.location < self.text.count
...
extension UITableView {
    @discardableResult
    func removeHeaderView() -> UITableView {
    ...
    @discardableResult
    func hidesSectionFooter() -> UITableView {
        self.sectionFooterHeight = CGFloat.compatibleLeastNormalMagnitude
        return self
    }
    
    @discardableResult
    func adjustBottomInset() -> UITableView {
        if #available(iOS 11, *) {
    ...
    @discardableResult
    func hidesSectionFooterAndAdjustBottomInset() -> UITableView {
        return self.hidesSectionFooter().adjustBottomInset()
    }
}
...
extension UIStackView {
    func add(arrangedSubviews: UIView...) {
        self.add(arrangedSubviews: arrangedSubviews)
    }
    
    func add<S>(arrangedSubviews:S) where S : Sequence, S.Element : UIView {
